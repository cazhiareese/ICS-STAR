      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="order-1 sm:order-none">
          <div className="bg-gray-100 p-4 rounded-xl shadow text-center">
            <h2 className="text-xl font-semibold">Info / Announcements</h2>
            <p className="text-sm text-gray-600">This area can be used for announcements or stats.</p>
          </div>
        </div>

        {/* Search Bar */}
        <div className="order-2 sm:col-span-2 sm:order-none">
          <input
            type="text"
        ...
        {/* Donation Data */}
        <div className="order-3 sm:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
          {filteredData.length > 0 ? (
            filteredData.map((drive, index) => (
              <div key={index} className="p-4 border rounded-xl shadow bg-white">
                ...
                <p className="text-xs text-gray-400 mt-1">📅 {new Date(drive.created_at).toLocaleDateString()}</p>
              </div>
            ))
          ) : (
