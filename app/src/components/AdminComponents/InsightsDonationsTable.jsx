...
function InsightsDonationsTable({ data, loading = false }) {
  ...
  const renderRows = () => {
    ...
    const dataRows = data.slice(0, rowsPerPage).map((donation, index) => (
      ...
      >
        <td className="py-3 px-6 flex items-center gap-2 font-satoshi-bold">
          {donation['Date Donated'] || 'N/A'}
        </td>
        <td className="py-3 px-6">{donation['Donation Drive'] || 'N/A'}</td>
        <td className="py-3 px-6">{donation['Name'] || 'N/A'}</td>
        <td className="py-3 px-6">{donation['Donation Type'] || 'N/A'}</td>
        <td className="py-3 px-6">{donation['Donation Details'] || 'N/A'}</td>
      </tr>
    ));
    ...
    const emptyRows = Array.from({ length: rowsPerPage - dataCount }).map((_, index) => (
      <tr
        ...
        style={{ minHeight: '48px' }}
      >
        <td className="py-3 px-6 flex items-center gap-2 font-satoshi-bold">N/A</td>
    ...
    <table className="w-full table-fixed">
      ...
      <thead>
        <tr className="text-left text-xs text-primary font-satoshi-regular">
          <th className="pt-4 pb-2 px-6 w-1/10">Date Donated</th>
          <th className="pt-4 pb-2 px-6 w-3/10">Donation Drive</th>
          <th className="pt-4 pb-2 px-6 w-2/10">Name</th>
          <th className="pt-4 pb-2 px-6 w-2/10">Donation Type</th>
          <th className="pt-4 pb-2 px-6 w-2/10">Donation Details</th>
        </tr>
      </thead>
